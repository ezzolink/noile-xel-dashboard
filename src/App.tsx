...
function App() {
  ...
  const tabs = [
    ...
    { id: 'projects', label: 'Projetos', icon: Network },
    ...
    return (
        <div className="flex flex-col h-screen bg-[#030308] items-center justify-center px-10">
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center w-full">
                ...
                <h1 className="text-white text-lg font-black uppercase tracking-[5px] mb-2">Comando Central</h1>
                <p className="text-slate-500 text-[10px] uppercase tracking-widest mb-10 text-center">Elias Sebastião Enterprise</p>
                ...
                <div className="w-full relative">
                    ...
                    />
                    <button onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-600 hover:text-accent">
                        {showPass ? <EyeOff size={18} /> : <Eye size={18} />}
                    </button>
                </div>
                
                <button onClick={handleLogin} className="w-full mt-6 bg-accent text-black font-black py-5 rounded-2xl uppercase tracking-[3px] text-xs shadow-xl shadow-accent/20 active:scale-95 transition-all">Aceder Hub</button>
            </motion.div>
        </div>
    );
  }
    ...
    <div className="flex flex-col h-screen max-w-md mx-auto relative px-5 selection:bg-accent selection:text-black">
      <header className="pt-10 pb-6 flex justify-between items-center px-2">
        <div className="flex flex-col">
            <h1 className="text-accent text-xs tracking-[8px] font-extralight uppercase opacity-80">E.S. Hub</h1>
            <div className="flex items-center gap-2 mt-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_10px_#10b981]"></span>
                <span className="text-[7px] text-slate-500 uppercase font-black tracking-[2px]">Neural Matrix Online</span>
            </div>
        </div>
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="relative group">
                ...
                <div className="relative bg-black border border-white/5 p-5 rounded-2xl flex items-start gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        <span className="text-[8px] font-black text-accent/60 uppercase tracking-[2px]">Neural Simulation</span>
          ...
          {activeTab === 'projects' && (
            <motion.div key="proj" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              <div className="px-2">
                 <h2 className="text-[9px] text-slate-500 uppercase font-black tracking-[4px] mb-6 font-mono">Project_Tracking_System</h2>
              </div>
              {projects.map(p => (
                <div key={p.id} className="glass-card p-7 relative overflow-hidden group border-l-4 border-accent">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex flex-col">
                      <h2 className="text-xl font-bold flex items-center gap-3 text-slate-100">
                        {p.name} <Zap size={16} className="text-gold" />
                      </h2>
                  ...
                  <div className="mt-8 pt-6 border-t border-white/5 flex justify-between items-center">
                    ...
                    <button className="bg-white/5 text-slate-300 text-[8px] px-6 py-3 rounded-2xl font-black border border-white/5 hover:bg-accent hover:text-black transition-all uppercase tracking-[2px]">Gerir Projecto</button>
                  </div>
                </div>
              ))}
            </motion.div>
          ...
          {activeTab === 'agenda' && (
            <motion.div key="age" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              <div className="glass-card p-6 border-l-4 border-red-500 bg-gradient-to-r from-red-500/[0.03] to-transparent text-red-500">
                <h2 className="text-[9px] uppercase font-black tracking-[3px] flex items-center gap-2 mb-5">
                  <AlertCircle size={12} /> Alertas Críticos
                </h2>
                <div className="space-y-4 font-mono text-[10px]">
                  <div className="flex justify-between items-center text-slate-300">
                    <span>Fatura Evento 10/02</span>
                    <span className="bg-red-500/10 px-2 py-1 rounded font-black">Atrasado</span>
                  </div>
                </div>
              </div>
              <div className="glass-card p-7">
                <h2 className="text-[9px] text-slate-500 uppercase font-black tracking-[3px] mb-8 font-mono">Sequential_Roadmap</h2>
                <div className="space-y-8">
                  {upcomingEvents.map((ev, i) => (
                    <div key={i} className="flex justify-between items-center group relative">
                      <div className="flex items-center gap-6">
                        <div className="w-12 h-12 rounded-full bg-black border-2 border-white/5 flex flex-col items-center justify-center group-hover:border-accent transition-all duration-500">
                      ...
                      {ev.value > 0 && (
                          <button onClick={() => generateInvoice(ev)} className="p-3.5 bg-white/5 rounded-2xl hover:bg-accent transition-all text-accent hover:text-black shadow-xl"><Download size={15} /></button>
                      )}
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'system' && (
            <motion.div key="sys" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pt-4">
              <div className="glass-card p-7 bg-black shadow-2xl border-white/10 relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-8 pb-4 border-b border-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[12px] text-white font-black uppercase tracking-[3px]">Kernel Monitor</span>
                            <span className="text-[8px] text-emerald-500 font-black tracking-[4px]">STABLE.2.6.5-ONLINE</span>
                        </div>
                    </div>
                    <button 
                ...
                <div className="space-y-4 text-[10px] font-mono text-accent/60 leading-relaxed max-h-96 overflow-y-auto pr-2 scrollbar-hide">
                  {logs.map(l => (
                    <div key={l.id} className="flex gap-4 border-l border-white/5 pl-4 py-2">
                        <span className="text-slate-800 text-[8px] font-bold">[{new Date(l.created_at).toLocaleTimeString()}]</span>
              ...
              <div className="glass-card p-6 bg-gradient-to-br from-white/5 to-transparent border-white/10 text-center">
                 <h3 className="text-[10px] text-white font-black uppercase tracking-[5px] mb-4">Fiscal Data</h3>
                 <div className="bg-black/60 p-5 rounded-2xl font-mono text-[10px] text-accent/80 border border-white/5 space-y-2 text-left">
                    <p className="flex justify-between"><span>CLIENT:</span> <span className="text-white font-bold tracking-tighter">ACELERADOR EMPRESARIAL</span></p>
                    <p className="flex justify-between"><span>NIF:</span> <span className="text-white font-bold">5001970658</span></p>
                </div>
                <p className="text-[7px] text-slate-600 uppercase font-black tracking-widest mt-6">Elias Sebastião Enterprise © 2026</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {loading && (
        <div className="absolute inset-0 bg-black/95 backdrop-blur-3xl z-[3000] flex flex-col items-center justify-center text-center">
            <Activity className="text-accent animate-spin mb-4" size={40} />
            <span className="text-[10px] text-accent font-mono tracking-[10px] animate-pulse uppercase font-black">Syncing Hub</span>
        </div>
      )}
    </div>
  );
